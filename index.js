...
testFlavor = getFlavorByIndex(originalFlavors, 5);
console.log(testFlavor);
...
function removeFlavorByName(flavorList, remFlavor){

    for(i = 0; i < flavorList.length; i++) {
        if(flavorList[i] == remFlavor) {
...
function filterByWord(thisArray, word){
    
    let filteredArray = [];
    
    for(let i = 0; i < thisArray.length; i++) {
        if(thisArray[i].includes(word)) {
            filteredArray.push(thisArray[i]);
        }
    }
...
let listOfChocolateFlavors = [];
listOfChocolateFlavors = filterByWord(originalFlavorsCopy, "Chocolate");
console.log(listOfChocolateFlavors);
